...
/**
 * @desc: 扩展 SimpleChannelInboundHandler，并处理 TextWebSocketFrame 消息
 * @author: zhongqionghua
 * @create: 2019/6/21 13:41
 */
public class TextWebSocketFrameHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
	...
	public void userEventTriggered(ChannelHandlerContext ctx,
		...
		//如果该事件表示握手成功，则从该 ChannelPipeline 中移除HttpRequestHandler，因为将不会接收到任何HTTP消息了
		if (evt == WebSocketServerProtocolHandler.ServerHandshakeStateEvent.HANDSHAKE_COMPLETE) {
			ctx.pipeline().remove(HttpRequestHandler.class);
			//(1) 通知所有已经连接的 WebSocket 客户端新的客户端已经连接上了
			// (当前的channel是不会通知的,因为当前的channel在下一行才被添加的group中)
