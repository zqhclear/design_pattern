...
/**
 * 复合享元角色类 复合享元对象是由单纯享元对象通过复合而成的，因此它提供了add()这样的聚集管理方法。
 * 由于一个复合享元对象具有不同的聚集元素，这些聚集元素在复合享元对象被创建之后加入，
 * 这本身就意味着复合享元对象的状态是会改变的，因此复合享元对象是不能共享的。复合享元角色
 * 实现了抽象享元角色所规定的接口，也就是operation()方法，这个方法有一个参数，代表复合享元对象的外蕴状态。
...
public class ConcreteCompositeFlyweight implements Flyweight {
	...
	public void outOfSideOperate(String typeStatus) {
		for (Object o : files.keySet()) {
			if (files.get(o) != null){
				files.get(o).outOfSideOperate(typeStatus);
			}
		}
